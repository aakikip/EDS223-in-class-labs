...
world_elevation <- read_stars(here::here("data", "worldelevation.tif"))
...
tm_shape(world_elevation) + # plot global elevations
  ...
  tm_dots()+
  tm_text("name", size = 0.7, col = "blue")
...
tm_shape(world_vector) +
  tm_borders() +
tm_shape(world_cities, is.main = TRUE) + #plotting world_cities last, but set as main shape
  tm_dots() +
  tm_text("name")
```
...
```{r}
tm_shape(world_elevation, bbox = c(-15, 35, 45, 65)) + # add bounding to restrict extent of map
  tm_raster() +
tm_shape(world_vector) + 
  tm_borders()
```
...
```{r}
# fill polygon with a color pallette representing a variable
tm_shape(world_vector) +
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy", 
              ...
              #fill.scale = tm_scale(values = "brewer.Yl0rBr")
...
tm_shape(world_vector) +
  tm_polygons(fill = "wb_income_region",
              ...
              fill = "gdp_per_cap",
              #fill.scale = tm_scale(breaks = c(0, 5000, 10000, 20000, 40000, 80000))
...
tmap_save(map1, here("tmap-example.png"), device = ragg::agg_png)

# tmap_save(map1, here("tmap-example.jpg")
```
